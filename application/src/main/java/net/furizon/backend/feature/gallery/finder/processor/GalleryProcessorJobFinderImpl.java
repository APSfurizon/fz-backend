...
public class GalleryProcessorJobFinderImpl implements GalleryProcessorJobFinder {
    @Qualifier(GALLERY_HTTP_CLIENT)
    private final HttpClient galleryHttpClient;

    @Override
    @NotNull
    public Optional<GalleryProcessorJob> getJobByReqId(long reqId) {
        log.info("Retrieving job {}", reqId);
