...
uploadDirs = [
	...
	#{"event": "furizon/zenith",	"user": 281,			"path": "Furizon Zenith 2025/Furizon Zenith 2025 - User uploads/luca/jpg/"},
...
## UNCOMMENT FOR UPLOADING FROM DIRS
...
## UNCOMMENT FOR UPLOADING SPECIFIC FILES. You may need to adjust the code
#retryFiles = set()
#with open("log_gallery_upload_UPLOADERRORS_cleaned.log", "r") as logs:
#	for line in logs:
#		if '**[ERROR] Uploading file' in line:
#			parts = line.split("'")
#			if len(parts) >= 2:
#				fileName = parts[1]
#				retryFiles.add(fileName)
#				tsPrint(f"**[DEBUG] Adding '{fileName}' to retry list based on log analysis.")
#with open("galleryUpload.json", "r") as f:
#    allFiles = json.load(f)
#for eventId, uploads in allFiles.items():
#	if (eventId not in eventToUploads):
#		eventToUploads[eventId] = []
#	event: list = eventToUploads[eventId]
#	for upload in uploads:
#		if upload["fileName"] in retryFiles:
#			tsPrint(f"**[DEBUG] Adding '{upload['fileName']}' to upload list based on log analysis.")
#			event.append(upload)
#			totalPhotos += 1
...
for eventId, uploads in eventToUploads.items():
	for upload in uploads:
		...
		try:
			...
			uploadKey = (upload["photographerId"], upload["rejected"])
...
# TODO Reupload single files with error
# TODO Find files with rejected formats, see if I can upload them
# DONE Manually reject files (a bug approved everything)